...
module.exports = new Command('search', async (message, args, context) => {
    this.client = context.client
        ...
        if(!voiceChannel.channelID) return message.channel.createMessage(`${this.client.emojis.cross} | You must be in a Voice Channel to use \`search\` command.`);
        if(!args.join(" ")) return message.channel.createMessage(`${this.client.emojis.cross} | You must provide a Search Query or an URL to use \`search\` command.`);
